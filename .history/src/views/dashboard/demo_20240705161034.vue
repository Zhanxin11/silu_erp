    ...
    <div class="filter-container">
      ...
      <el-table style="width: 100%; height: 100%; margin-top: 20px" :data="tableData" border class="computed-table"
        ...
        <el-table-column fixed="right" label="汇总" width="150" align="center">
          <template slot-scope="scope">
            <div :id="'main_'+scope.row" style="width:70%;height:30px;margin: 0 auto;"></div>
          </template>
        </el-table-column>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      initEcharts() {
        var stateList = []
        stateList = this.tableData.map(item => {
          return item.values.map(i => {
            return i.value
          })
          console.log(item.values)
          stateLIst.push(item.values)
        })
        console.log(this.tableData, stateList)
        for (let i = 0; i < statelist.length; i++) {
          let myChart = echarts.init(document.getElementById("main_" + statelist))
          // 绘制图表
          myChart.setOption({
            xAxis: {
