      ...
      initEcharts() {
        ...
        this.$nextTick(() => {
          for (let i = 0; i < this.tableData.length; i++) {
            let myChart = echarts.init(document.getElementById("main_" + this.tableData[i].sum))
            console.log(myChart, 111, this.tableData)
            if (chartDom) {
              let myChart = echarts.init(chartDom)

              // 假设 this.tableData[i].sum 是一个包含 Y 轴值的数组
              let xAxisData = [] // 用于 X 轴的数据
              for (let j = 0; j < this.tableData[i].sum.length; j++) {
                xAxisData.push(j.toString()) // 简单的索引转换为字符串（如果不需要，也可以直接用 j）
              }
            }
            // 绘制图表
            myChart.setOption({
            ...
            console.log(this.tableData[i].sum, 'zzz')
