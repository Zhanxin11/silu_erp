 ...
 * @LastEditTime: 2024-07-16 16:34:03
    ...
    methods: {
      ...
      selectPro(val) {
        ...
        if (!val.includes('全选') && val.length === selectItem.length) {
          ...
          val= this.BusinesValue.filter((item) => {
            return item !== '全选'
          })
        } else if (          val= this.BusinesValue.filter((item) => {
includes('全选') && val.length < this.BusinesOptions.length + 1) {
