...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      // 遍历productData中的data数组
      this.productData.data.forEach(item => {
        // 检查item中是否存在pic_field和thumbnail_field
        if (item.pic_field && item.thumbnail_field) {
          // 将它们合并到一个新数组中，并将这个新数组添加到merged数组中
          merged.push([item.pic_field, item.thumbnail_field]);
        }
      });
