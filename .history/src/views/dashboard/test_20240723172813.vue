 ...
 * @LastEditTime: 2024-07-23 17:27:41
    ...
    methods: {
      ...
      selectPro(val, num) {
        ...
        this.tableData = this.tableData.filter(item => {
          // 假设 selectOption 是一个包含可能 SPU 值的数组
          return !selectOption.includes(item.SPU) // 如果想排除特定的 SPU 值
          // 或者，如果你想保留特定的 SPU 值
          // return selectOption.includes(item.SPU);
        })
        console.log(selectOption, this.tableData)
        // this.tableData = this.tableData.filter(item => {
        //   console.log(item.SPU, selectOption, val, 'true1')
        //   // return !selectOption.includes(item.SPU)
        // })
