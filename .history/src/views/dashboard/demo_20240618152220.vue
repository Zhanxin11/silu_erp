  ...
  <div class="app-container">
    <div class="filter-container">
      <el-row>
        ...
        <el-select style="padding: 0 10px" v-model="teamValues" multiple filterable placeholder="请选择团队">
          <el-option v-for="item in teamOptions" :key="item.value" :label="item.label" :value="item.value">
          </el-option>
        </el-select>
        <el-select v-model="OperationValues" multiple filterable placeholder="请选择运营">
      ...
      <el-table style="width: 100%; height: 100%; margin-top: 20px" :data="tableData" border class="computed-table"
        ...
        <el-table-column v-for="(time, index) in times" :key="index" :label="time" :header-row-style="headerCellStyle"
          ...
          <template slot-scope="scope">
            <span class="inner_span" @click="changeValue(scope.$index,scope.row)"
              v-if="!scope.row.values[index].isShow"> {{
              scope.row.values[index].value}}</span>
            <el-input v-else v-model="scope.row.values[index].value"></el-input>
          </template>
        </el-table-column>
      </el-table>
    </div>
    ...
    methods: {
      ...
      cellClick(row, column, cell, event) {
        ...
        this.tableData.forEach(item => {
          console.log(item.index == row.index, row.index)
          if (item.index == row.index && item.values[index] == column.index) {
            item.values[index].isTure = ture
            console.log(item.values, Array.isArray(item.values), '确定列')

            // item.values.forEach(i => {
            //   console.log(i.index == column.index, i, i.index, column.index, '确定行')
            //   if (i.index === column.index) {
            //     i.isShow = true
            //     console.log(i)
            //   }
            // })
      ...
      setStyle({ row, column, rowIndex, columnIndex }) {
        ...
        if (columnIndex === 0 && rowIndex === 0) {
          row.values.forEach(item => {
            if (item >= 30) {
              return styleCol = 'background: pink !important;color:white'
            }
          })
        }
      },
      ...
      changeValue(index, row) {
        console.log(index, row.values, 'chenge')
      }
    }
  }
</script>
