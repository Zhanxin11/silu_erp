      ...
      <el-table style="width: 100%; height: 100%; margin-top: 20px" :data="tableData" border class="computed-table"
        ...
        <el-table-column v-for="(time, index) in times" :key="index" :label="time" :header-row-style="headerCellStyle"
          ...
          <template slot-scope="scope">
            ...
            <el-input v-else @blur="handeBlur(scope.row, index)" v-model="scope.row.values[index].value"></el-input>
...
<script>
  export default {
    ...
    methods: {
      ...
      // 失焦保存
      handeBlur(scope, row) {
        scope.row.values[scope.$index].isCheck = false
        console.log(scope, '失焦')
      },
      // 设置单元格样式
