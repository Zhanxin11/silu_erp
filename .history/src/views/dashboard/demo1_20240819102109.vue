 ...
 * @LastEditTime: 2024-08-19 10:17:20
  ...
  <div class="app-container">
    <div class="filter-container">
      <el-row>
        ...
        <el-button type="primary" v-else @click="isCheck=false">取消编辑</el-button>
...
<script>
  ...
  this.tableData.forEach(item => {
    ...
    if (arr.includes(item.sku_field)) { // 直接检查 item.sku_field 是否在 arr 中
      item.date.forEach(dateItem => {
        dateItem.isCheck = true; // 只有当 item.sku_field 在 arr 中时，才设置 isCheck 为 true
      });
    }
  });
  // 注意：移除了不必要的 skuList 数组和 this.isCheck = true（除非它在其他地方有用途）
},
      // 下载
      downloadExcel() { },
      // 新增
