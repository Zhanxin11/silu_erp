 ...
 * @LastEditTime: 2024-07-17 13:34:49
    ...
    <div class="filter-container">
      <el-row>
        <el-select v-model="BusinesValue" class="select-Bus" multiple placeholder="请选择商号(必选)" filterable collapse-tags
          @change="selectPro(BusinesValue,1)" @remove-tag="removeTag">
        ...
        <el-select @change="selectPro(spuValues,4)" style="padding: 0 10px" v-model="spuValues" multiple collapse-tags
          filterable placeholder="请选择spu">
  ...
  export default {
    ...
    methods: {
      ...
      selectPro(val, num) {
        ...
        if (!selectValue.includes('全选') && selectValue.length === this.BusinesOptions.length) {
          selectValue.unshift('全选')
        } else if (selectValue.includes('全选') && selectValue.length !== this.BusinesOptions.length) {
          selectValue = selectValue.filter((item) => {
            return item !== '全选'
          })
        } else if (selectValue.includes('全选') && val.length < this.BusinesOptions.length + 1) {
          this.BusinesValue = selectValue.filter((item) => {
            return item !== '全选'
          })
        }
        if (num == 1) {
