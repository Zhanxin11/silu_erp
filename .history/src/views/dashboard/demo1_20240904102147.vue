 ...
 * @LastEditTime: 2024-09-04 09:05:04
  ...
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-table id="table_out" style="width: 100%; height: 100%;margin-top: 20px" border :data="tableData"
        ...
        <el-table-column label="图片" prop='thumbnail_field'>
          <template slot-scope="scope">
            <div style="width: 100px;height: 100px;text-align: center;">
              <el-image :src="'http://121.41.171.227:8000/' + scope.row.thumbnail_field"
                :preview-src-list="scope.row.thumbnail"></el-image>
            </div>
          </template>
        </el-table-column>
    ...
    computed: {
      ...
      processedItems() {
        return this.tableData.map(item => {
          // 创建一个新对象，包含原始数据和新的images数组
          const newItem = []

          // 检查并添加pic_field（如果存在，则添加http前缀）
          if (item.pic_field) {
            newItem.push(`http://121.41.171.227:8000/${item.pic_field}`)
          }

          // 添加thumbnail_field
          if (item.thumbnail_field) {
            newItem.push(`http://121.41.171.227:8000/${item.thumbnail_field}`)
          }
      ...
      async initData() {
        ...
        try {
          ...
          this.tableData.map(item => {
            if (!Array.isArray(item.pic_field) && item.pic_field) {
