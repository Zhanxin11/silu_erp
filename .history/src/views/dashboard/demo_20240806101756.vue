      ...
      >
        ...
        >
          <template slot-scope="scope">
            <span
              class="inner_span"
              :style="classObje(scope.row.values[index].value, scope.$index)"
              @click="changeValue(scope.$index, scope.row)"
            ...
            <el-input
              v-else
              ref="inputs"
              @keydown.enter.native="handeBlur(scope.row, index)"
              v-model="scope.row.values[index].value"
            ></el-input>
          </template>
        </el-table-column>
        <el-table-column
        ...
        >
          <template slot-scope="scope">
            <div
              ...
              style="width: 95%; height: 50px; margin: 0 auto"
            >
              <!-- {{ scope.$index }} -->
            </div>
          </template>
        ...
        <el-table-column fixed="right" label="操作" width="100" align="center">
          <template slot-scope="scope">
            ...
            >
            <el-button
              v-show="scope.row.parameter != 'GMV'"
              type="text"
              size="small"
              >删除</el-button
            >
    ...
    initEcharts() {
      this.$nextTick(() => {
        ...
        for (let i = 0; i < this.tableData.length; i++) {
          console.log(i)
          let myChart = echarts.init(document.getElementById("echarts_" + i))
          if (myChart) {
            console.log(myChart, 111, Array.isArray(this.tableData[i].sum), '获取dom成功')
          } else {
            console.log('huoqudomshibai1')
          }
          // 绘制图表
          myChart.setOption({
            xAxis: {
          ...
          // 监听窗口大小变化
          console.log(this.tableData[i].sum, this.times, 'zzz')
