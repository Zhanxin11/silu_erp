      ...
      <el-table style="width: 100%; height: 100%; margin-top: 20px" :data="tableData" border class="computed-table"
        ...
        <el-table-column v-for="(time, index) in times" :key="index" :label="time" :header-row-style="headerCellStyle"
          ...
          <template slot-scope="scope">
            ...
            <el-popover v-else placement="top" width="160" v-model="visible">
              <el-input  @blur="handeBlur(scope)" v-model="scope.row.values[index].value"></el-input>
            </el-popover>
          </template>
        </el-table-column>
      </el-table>
...
<script>
  export default {
    ...
    methods: {
      ...
      // 失焦保存
      handeBlur(scope) {
        scope.row.values[scope.$index].isCheck = true
