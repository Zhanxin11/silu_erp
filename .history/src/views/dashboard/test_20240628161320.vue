 ...
 * @LastEditTime: 2024-06-28 16:13:20
    ...
    methods: {
      ...
      handleDelete(index, row) {
        ...
        this.$confirm(`你确定删除${row.name}, 是否继续?`, '提示', {
          ...
          // 发送 API 请求来删除服务器上的数据
          this.deleteDataApi(row.id).then(response => {
            // 如果 API 请求成功，更新本地数据
            if (response.data.success) { // 假设 API 返回了一个包含 success 字段的响应
              ...
              this.$message({
                type: 'error',
                message: '删除失败: ' + response.data.message // 假设 API 返回了一个包含 message 字段的错误信息
              })
            }
          }).catch(error => {
            // 处理 API 请求中的网络错误或其他错误
            this.$message({
              type: 'error',
              message: '网络错误，删除失败'
            })
