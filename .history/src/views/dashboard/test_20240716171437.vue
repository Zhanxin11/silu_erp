 ...
 * @LastEditTime: 2024-07-16 17:14:23
  ...
  <div class="app-container">
    <div class="filter-container">
      <el-row>
        ...
        <el-select @change="selectPro(spuValues,Operations)" style="padding: 0 10px" v-model="spuValues" multiple
          collapse-tags filterable placeholder="请选择spu">
          <el-option label="全选" value="全选" @click.native="selectAll(2)">全选</el-option>
  ...
  export default {
    ...
    methods: {
      ...
      selectPro(val, num) {
        ...
        if (!val.includes('全选') && val.length === selectItem.length) {
          val.unshift('全选')
        } else if (val.includes('全选') && val.length !== selectItem.length) {
          val = val.filter((item) => {
            return item !== '全选'
          })
        } else if (val.includes('全选') && val.length < selectItem.length + 1) {
          val = val.filter((item) => {
            return item !== '全选'
          })
        }
        if (val == selectValue) {
          console.log(val == this.BusinesValue, 'val')
          this.BusinesValue = selectValue
          // this.BusinesOptions = selectItem
          console.log(1, selectValue, selectItem)
        } else {
