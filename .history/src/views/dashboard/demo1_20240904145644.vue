 ...
 * @LastEditTime: 2024-09-04 14:56:40
  ...
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-table id="table_out" style="width: 100%; height: 100%;margin-top: 20px" border :data="tableData"
        ...
        <el-table-column label="图片" prop='thumbnail_field' align="center">
          <template slot-scope="scope">
            <div style="height: 100px; display: flex;justify-content: center; ">
              <el-image :src="'http://121.41.171.227:8000/' + scope.row.thumbnail_field"
                :preview-src-list="scope.row.pic_field" fit="fill"></el-image>
            </div>
          </template>
        </el-table-column>
        <el-table-column v-for="(item,index) in dates" :key="item" :label="item" :prop="item">
          <template slot-scope="scope">
            <span v-if="!scope.row.date[index].isTrue">{{
              scope.row.date[index][item]}}</span>
...
// 通过blob的方式进行下载
download (_blob: any, filename: any) => {
...
————————————————
      ...
      原文链接：https://blog.csdn.net/XUANEER/article/details/137824665
    },
    mounted() {
      this.initData()
    },
