 ...
 * @LastEditTime: 2024-08-19 10:02:09
  ...
  export default {
    ...
    methods: {
      ...
      changeList() {
        ...
        this.isCheck = true
        this.tableData.forEach(item => {
          skuList.push(item.sku_field)
          let arr = this.delSkuList.split(',')
          arr.forEach(i => {
            console.log(i, skuList)
            if (skuList.indexOf(i)) {
              item.date.forEach((i, k) => {
                i.isCheck = true
              })
            }
          })
          // console.log(skuList, arr)
          // console.log(arr, arr.indexOf(item.sku))
          // item.date.forEach((e, i) => {
          //   e[i].is Check
          // })
        })
      },
