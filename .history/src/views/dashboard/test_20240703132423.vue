 ...
 * @LastEditTime: 2024-07-03 13:20:07
...
<script>
  ...
  export default {
    ...
    // 如果存在固定列，我们需要创建一个新的表格元素来仅包含非固定部分  
    if (fix) {  
        // 克隆原始表格，但排除固定部分  
        let cloneTable = table.cloneNode(true);  
        let fixedWrapper = cloneTable.querySelector(".el-table__fixed-wrapper");  
        if (fixedWrapper) {  
            fixedWrapper.remove(); // 移除固定列的包装器  
        }  
  
        // 使用处理后的表格克隆来创建Workbook  
        wb = XLSX.utils.table_to_book(cloneTable);  
    } else {  
        // 没有固定列，直接使用原始表格  
