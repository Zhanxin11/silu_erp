...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 添加标记
      addIsTrue() {
        console.log(this.tableData) // 查看原始 tableData

        // 遍历 tableData 数组并更新每个 item
        this.tableData = this.tableData.map(item => {
          // 浅拷贝当前 item
          const newItem = { ...item }

          // 如果 item.date 存在且是对象
          if (newItem.date && typeof newItem.date === 'object' && newItem.date !== null) {
            // 遍历 date 对象的每个键
            newItem.date = Object.keys(newItem.date).reduce((acc, key) => {
              // 添加 isCheck: false 到每个属性
              acc[key] = { ...newItem.date[key], isCheck: false }
              return acc
            }, {})
          }
