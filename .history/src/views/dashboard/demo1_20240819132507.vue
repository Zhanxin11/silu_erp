 ...
 * @LastEditTime: 2024-08-19 13:20:33
      ...
      <el-table style="width: 100%; height: 100%;margin-top: 20px" border :data="tableData" id="table"
        ...
        <el-table-column v-for="(item,index) in dates" :key="item" :label="item" :prop="item">
          <template slot-scope="scope">
            <span v-if="!scope.row.date[index].ischeck">{{
              scope.row.date[index][item]}}---{{scope.row.date[index].ischeck}}</span>
      ...
      async initData() {
        ...
        try {
          ...
          this.tableData.map(item => {
            return item.date = this.changeTableData(item)
          })
        ...
        this.$nextick(() => {
          this.tableData.map(item => {
            ...
            if (arr.includes(item.sku_field)) {
              return item.date.map(dateItem => {
                return dateItem.ischeck = true
              })
            }
