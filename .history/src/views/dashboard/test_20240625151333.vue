 ...
 * @LastEditTime: 2024-06-25 13:57:15
  ...
  <div class="app-container">
    ...
    <el-dialog title="增加" :visible.sync="dialogFormVisible">
      <el-form :model="formData">
        <el-form-item v-for="(value,key, index) in formData" label-width="100px" :key="index" :label="key||key!=id"
          :prop="value">
...
<script>
  import { getDemo, CreateDemo } from '@/api/demo'
  export default {
    components: {},
    props: {},
    data() {
    ...
    methods: {
      ...
      // 保存
      saveForm({ SPU, sku计数, 开始时间, 结束时间, 销量 }) {
        // const form = {}
        ...
        formData.append(SPU)
        formData.append(sku计数)
        formData.append(开始时间)
        formData.append(开始时间)
        formData.append(开始时间)
        CreateDemo({ SPU, sku计数, 开始时间, 开始时间, 开始时间 }).then(res => {   //调用接口进行保存
          console.log(res)
          if (res.code !== 200) {
          ...
          console.error(error)
          this.$message.error("保存时发生错误，请稍后再试！")
