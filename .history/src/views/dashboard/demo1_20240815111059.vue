 ...
 * @LastEditTime: 2024-08-15 11:08:51
    ...
    computed: {
      ...
      dates() {
        if (this.tableData && this.tableData[0]?.date) {
          ...
          console.log(Object.keys(this.tableData[0].date), 'oj')
        ...
        this.tableData.forEach(item => {
          ...
          if (item.date && typeof item.date === 'object' && item.date !== null) {
            ...
            Object.keys(item.date).forEach(key => {
              ...
              // 更新 date 对象中的值
              // 注意：这里我们直接将整个值替换为一个新对象，这可能不是您想要的
              // 如果您想要保留原始结构，请考虑使用其他方式（如下面的示例）
              item.date[key] = newValue

              // 如果您不希望改变原始结构，而是想要一个包含原始值和 isCheck 的新对象数组
              // 您可以创建一个新属性来存储这些对象，而不是替换原始值
              // 例如：
              // if (!('checkedValues' in item)) {
              //   item.checkedValues = {};
              // }
              // item.checkedValues[key] = newValue;
            })
          }
        })
