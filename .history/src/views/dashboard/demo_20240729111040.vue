    ...
    <div class="filter-container">
      ...
      <el-table style="width: 100%; height: 100%; margin-top: 20px" :data="tableData" border class="computed-table"
        ...
        <el-table-column fixed="right" label="汇总" width="150" align="center">
          <template slot-scope="scope">
            <div :id="'main_'+scope.row.parameter" style="width:70%;height:30px;margin: 0 auto;">{{scope.row}}</div>
          </template>
        </el-table-column>
  ...
  export default {
    ...
    methods: {
      ...
      initEcharts() {
        ...
        console.log(this.stateList.length, 'this.stateList.lengt')
        for (let i = 0; i < this.stateList.length; i++) {
          let myChart = echarts.init(document.getElementById("main_"+this.tableData[i].pa))
          // console.log(myChart)
          // 绘制图表
          myChart.setOption({
            xAxis: {
