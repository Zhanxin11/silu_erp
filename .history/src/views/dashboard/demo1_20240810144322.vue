 ...
 * @LastEditTime: 2024-08-10 14:43:22
...
<script>
  ...
  this.tableData = this.tableData.map(item => {
    ...
    Object.keys(item).forEach(key => {
      ...
      // 如果当前属性是 date 且是一个对象
      if (key === 'date' && typeof item[key] === 'object' && item[key] !== null) {
        // 遍历 date 对象的每个属性
        newItem[key] = Object.keys(item[key]).reduce((acc, d) => {
          // 复制 date 对象的每个属性，并添加 isCheck: false
          acc[d] = { ...item[key][d], isCheck: false };
          return acc;
        }, {});
      } else {
        // 对于非 date 属性，直接添加 isCheck: false（如果适用）
        // 但通常，对于非对象类型的属性，这样做可能没意义
        // 这里我们假设所有非 date 的属性都不需要这样的处理
        // 如果需要，可以添加逻辑来检查并修改这些属性
      }
    });
    // 返回修改后的对象
    return newItem;
