...
<script>
  export default {
    ...
    methods: {
      ...
      //     for (let [key, value] of Object.entries(item.values)) {
      ...
      addTrue() {
        this.tableData = this.tableData.map(item => {
          let isCheck = false // 在这里声明 isCheck 变量
          for (let [key, value] of Object.entries(item.values)) {
            console.log(key, value)
            if (value === true) { // 检查值是否为 true
              isCheck = true // 如果找到 true，设置 isCheck 为 true
              break // 可以选择退出循环，因为已经找到了 true
            }
          }
          // 返回一个新的对象，其中包含原始的 item 属性和新添加的 isChecked 属性
          return {
            ...item, // 使用扩展运算符复制原始对象
            isChecked: isCheck // 添加 isChecked 属性
          }
        })
        console.log(this.tableData, 'td')
      }
