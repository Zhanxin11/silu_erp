 ...
 * @LastEditTime: 2024-07-17 10:03:58
    ...
    <div class="filter-container">
      <el-row>
        <el-select v-model="BusinesValue" class="select-Bus" multiple placeholder="请选择商号(必选)" filterable collapse-tags
          @change="selectPro(BusinesValue,BusinesOptions)" @remove-tag="removeTag">
        ...
        <el-select @change="selectPro(spuValues,spuOptions)" style="padding: 0 10px" v-model="spuValues" multiple
          collapse-tags filterable placeholder="请选择spu">
    ...
    methods: {
      ...
      selectAll(val) {
        ...
        if (val == 1) {
          ...
          console.log(1, selectValue, selectItem)
        } else {
          selectValue = this.spuValues
          selectItem = this.spuOptions
          console.log(2, selectValue, selectItem)
      ...
      //     console.log(this.BusinesValue, this.BusinesOptions)
      //     this.BusinesValue = []
      ...
      // },
      // 商号下拉
      selectPro(val, options) {
        let selectItem
        let selectValue
        console.log(val, options, 'shanghao')
        if (val == this.BusinesValue) {
          selectValue = val
          selectItem = options
          console.log(1)
        } else {
        ...
        if (!selectValue.includes('全选') && selectValue.length === options.length) {
          selectValue.unshift('全选')
        } else if (selectValue.includes('全选') && selectValue.length !== options.length) {
          selectValue = selectValue.filter((item) => {
            return item !== '全选'
          })
        } else if (selectValue.includes('全选') && selectValue.length < options.length + 1) {
          selectValue = val.filter((item) => {
            return item !== '全选'
          })
        }
        if (val == selectValue) {
          console.log(val == selectValue, 'val')
          this.BusinesValue = val
          console.log(1, selectValue, selectItem)
        } else {
          this.spuValues = val
          console.log(val == this.BusinesValue, 'val1')
          console.log(2, selectValue, selectItem)
        }
      },
      // selectPro(val) {
      //   let selectItem
      //   console.log(val, 'shanghao')
      //   if (val == this.BusinesValue) {
      //     selectItem = this.BusinesOptions
      //     console.log(1)
      //   } else {
