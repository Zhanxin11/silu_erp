 ...
 * @LastEditTime: 2024-08-15 11:01:29
    ...
    <div class="filter-container">
      ...
      <el-dialog class="dialog-box" title="批量上传" width="25%" :visible.sync="uploadShowDialog">
        ...
        <el-upload class="upload-demo" ref="upload" action="#" :multiple="false" :show-file-list="true"
          ...
          :on-change="handleUploadChange" :limit="5">
          <div class="border">
    ...
    methods: {
      ...
      addIsTrue() {
        ...
        this.tableData.map(item => {
          ...
          // // 如果 'date' 属性存在且是一个对象
          // if ('date' in item && typeof item.date === 'object' && item.date !== null) {
          //   // 遍历 date 对象的每个属性
          //   console.log(newItem, 'newItem.date')
          //   newItem.date = Object.keys(item.date).reduce((acc, d) => {
          //     console.log(acc[d], item.date[d], '...')
          //     // 复制 date 对象的每个属性，并添加 isCheck: false
          //     acc[d] = { ...item.date[d], isCheck: false }
          //     return acc
          //   }, {})
          // }
          console.log(item.date, 111)
          Object.values(item.date).map(i => {
            console.log(i)
            return {
              value: i,
              isCheck: false
        ...
        console.log(this.tableData) // 查看修改后的 tableData
      },
      // 批量修改
      changeList() {
