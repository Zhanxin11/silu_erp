 ...
 * @LastEditTime: 2024-10-24 10:13:57
      ...
      <el-form :inline="true" label-width="80px" :model="spuFromValues" ref="spuFromValues" :rules="spuFromRules">
        <el-form-item prop="category" label="大类">
          <el-select v-model="spuFromValues.category">
            <option v-for="i in categoryValue" :label="i" :value="i">
            </option>
          </el-select>
        </el-form-item>
      </el-form>
...
<script>
  ...
  export default {
    ...
    computed: {
      categoryValue() {
        let arr = []
        this.catSelectForm.category?.map(i => {
          arr.push(i.category)
        })
        console.log(this.catSelectForm.category, this.catSelectForm)
        return arr
      }
    },
    methods: {
