 ...
 * @LastEditTime: 2024-10-25 09:17:07
...
<template>
  <div class="app-container haplotype-detail default-scrollbar">
    ...
    <el-row>
      <el-col :span="6">
        <el-tree :data="convertToTreeData" show-checkbox node-key="id" default-expand-all :expand-on-click-node="false">
          <span class="custom-tree-node" slot-scope="{ node, data }">
            <span>{{ }}</span>
            <span>
              <el-button type="text" size="mini" @click="() => changeTree(data)">
                修改
              </el-button>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      convertToTreeData() {
        return this.catSelectForm.map(item => {
          const newNode = { label: item.category || item.category1 || item.category2, children: [] }
          if (item.childen && item.childen.length > 0) {
            newNode.children = convertToTreeData(item.childen)
            ...
            newNode.children = item.children.map(child => ({
              ...
              children: child.children ? convertToTreeData(child.children) : []
            }))
