 ...
 * @LastEditTime: 2024-10-25 11:25:34
...
<script>
  ...
  export default {
    ...
    methods: {
      async initData() {
        try {
          // 假设 getCatListApi 和 getCatSelectApi 返回 Promise
          this.tableData = await getCatListApi()
          this.loadingSpu = false
          this.catSelectForm = await getCatSelectApi()
          // 确保 catSelectForm 有数据后再调用 changeTree
          if (this.catSelectForm && this.catSelectForm.length > 0) {
            this.TreeData = this.changeTree(this.catSelectForm)
          } else {
            console.error('Failed to retrieve catSelectForm data')
            // 可能需要设置 TreeData 为一个默认值或空数组
          }
        } catch (error) {
    ...
    mounted() {
      this.TreeData = this.changeTree(this.catSelectForm)
      console.log(this.TreeData, this.catSelectForm, 1111)
