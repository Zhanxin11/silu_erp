 ...
 * @LastEditTime: 2024-10-16 13:32:18
...
<template>
  <div class="app-container haplotype-detail default-scrollbar">
    ...
    <el-table :data="tableData" id="table_out" border style="width: 100%" :header-cell-style="tableHeaderColor"
      ...
      <el-table-column prop='status' label="产品状态" :filters='statusFilterValue' :filter-method="filterDate">
        <template slot-scope="scope">
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      statusFilterValue() {
        const uniqueCategories = this.copyTableData.reduce((acc, i) => {
          const exists = acc.some(item => item.text === i.st)
          if (!exists) {
            acc.push({ text: i.spu, value: i.spu })
          }
          return acc
