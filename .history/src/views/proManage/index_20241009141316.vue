 ...
 * @LastEditTime: 2024-10-09 14:13:16
    ...
    <el-dialog title="新增" :visible.sync="dialogVisible" width="48%" :inline="true">
      ...
      <el-form :inline="true" label-width="80px" :model="proFormValues" ref="proFormValues" :rules="proRules">
        <el-form-item prop="store_brands_ops" label="商号">
          <el-cascader v-model="proFormValues.store_brands_ops" :props="{ checkStrictly: true,multiple: true, }"
            :options="opsValue" clearable>
            <!-- <template slot-scope="scope">
              <div></div>
            </template> -->
          </el-cascader>
  ...
  export default {
    ...
    data() {
      return {
        ...
        proRules: {
          store_brands_ops: [{ required: true, validator: this.validateName, trigger: 'blur' },]
        }
      }
      ...
      validator(rule, value, callback) {
        ...
        selected.forEach(path => {
          ...
          if (categoryValue === '运营') {
            ...
            // 检查是否有对应的运营人员
            if (opSet.has(storeValue + '_运营')) { // 假设运营人员的值以商号+'_运营'的形式存储，这里需要调整以匹配您的实际数据
              validStoreOpPairFound = true
              opSet.delete(storeValue + '_运营') // 匹配到后移除，避免重复计数
            }
          }
        })
        console.log(selected, storeSet, opSet, value)
        // 验证规则
