<template>
  <div class="app-container haplotype-detail default-scrollbar">
    <el-row style="padding-bottom: 20px">
      ...
      <el-button type="primary" @click="uploadSpuToExcel">上传Execl模板示例</el-button>
    </el-row>
    <el-row>
      <!-- <el-col :span="6">
    ...
    methods: {
      ...
      saveSpu(formName) {
        ...
        for (const key in this.spuFromValues) {
          ...
          this.spuFromValues[key] = value[key].replace(/^\s*|\s*$/g, '')
        ...
        if (this.spuFromValues.id) {
          this.$refs[formName].validate(valid => {
            if (valid) {
              putChangeCatApi(this.spuFromValues).then(response => {
