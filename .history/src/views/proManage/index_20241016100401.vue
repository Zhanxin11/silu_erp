 ...
 * @LastEditTime: 2024-10-16 09:52:54
...
<template>
  <div class="app-container haplotype-detail default-scrollbar">
    ...
    <el-table :data="tableData" id="table_out" border style="width: 100%" :header-cell-style="tableHeaderColor"
      @select="selectPro" v-loading="loadingPro">
  ...
  export default {
    ...
    data() {
      return {
        ...
        proRules: {
          ...
          category: [{ required: true, msg: '请输入大类', trigger: 'blur' }],
          category1: [{ required: true, msg: '请输入产品线', trigger: 'blur' }],
          category2: [{ required: true, msg: '请输入细分品线', trigger: 'change' }],
          spu: [{ required: true, validator: this.validateSpu, trigger: 'change' }],
          fasin: [{ required: true, msg: '请输入父asin', trigger: 'blur' }],
          asin: [{ required: true, msg: '请输入asin', trigger: 'blur' }],
          sku: [{ required: true, msg: '请输sku', trigger: 'blur' }],
          description: [{ required: true, msg: '请输入描述', trigger: 'blur' }],
          sell_date: [{ required: true, msg: '请输细开售时间', trigger: 'blur' }],
          status: [{ required: true, msg: '请选择产品状态', trigger: 'blur' }],
        }
      }
    },
    watch: {
    ...
    computed: {
      ...
      opFilterValue() {
        let brand = this.proForm.store_brands_ops?.reduce((acc, parent) => {
          parent.childen.forEach(child => {
            // 假设 child.brands 是一个包含字符串的数组
            child.brands?.forEach(brandText => { // 遍历 child.brands 数组
              // 使用 findIndex 或 some 方法检查 acc 中是否已经存在该 brandText
              if (!acc.some(item => item.text === brandText)) {
                acc.push({ text: brandText, value: brandText })
              }
            })
          })
          // 注意：这里的 console.log 会在每次遍历 parent.childen 后执行，可能会输出中间状态
          // 如果你只想看到最终结果，可以将这个 console.log 移到函数末尾
          return acc
        }, [])

        // 如果你只想在函数结束时看到最终结果，可以在这里打印
        // console.log(brand);
