 ...
 * @LastEditTime: 2024-10-14 15:26:11
    ...
    methods: {
      ...
      submitUpload() {
        ...
        if (!!this.file.file) {
          ...
          // batchuploadFile是接口
          postAddProApi(formData).then(res => {
            this.uploaProDialog = false
            this.handleQuery()
          })
        } else {
      ...
      downloadExcel() {
        ...
        let fileName = this.$route.name + new Date().toISOString().replace(/:/g, '-')
        table2excel(tableHeader, tableDatas, '产品资料')
      },
      // 打开新增dailog
      addpro() {
