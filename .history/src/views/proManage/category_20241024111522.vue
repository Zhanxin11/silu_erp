 ...
 * @LastEditTime: 2024-10-24 11:15:22
      ...
      <el-form :inline="true" label-width="80px" :model="spuFromValues" ref="spuFromValues" :rules="spuFromRules">
        <el-form-item prop="category" label="大类">
          <el-select v-model="spuFromValues.category">
            <option v-for="(i,index) in catSelectForm" :key="index" :label="i.category" :value="i.category">
            </option>
          </el-select>
        </el-form-item>
      </el-form>
  ...
  export default {
    ...
    computed: {
      categoryValue() {
        ...
        console.log(this.catSelectForm.category, this.catSelectForm, arr)
        return arr
      }
    },
    methods: {
      async initData() {
        ...
        this.catSelectForm.map(i => {
          console.log(i.category, 1)
        })
        console.log(this.catSelectForm[0]?.category)
