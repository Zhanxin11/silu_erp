 ...
 * @LastEditTime: 2024-09-28 16:23:15
    ...
    <el-dialog title="新增商号" :visible.sync="dialog" width="35%" :inline="true">
      <el-form :inline="true" label-width="80px" :model="firmFormValues" ref="firmFormValues" :rules="frimRules">
        ...
        <el-form-item prop="statusValue" label="商号状态">
          <el-select v-model="firmFormValues.nameValue" placeholder="请选择商号状态" filterable>
            <el-option v-for="(item,index) in firmForm.name" :key="index"
              :label="item.status === 'active' ? '在售' : '解约'" :value="item">
            </el-option>
          </el-select>
        </el-form-item>
      </el-form>
