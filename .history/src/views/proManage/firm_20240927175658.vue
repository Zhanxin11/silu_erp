 ...
 * @LastEditTime: 2024-09-27 17:47:27
...
<script>
  ...
  const property = column['property'];
  // 检查 row[property] 是否存在且不是空数组
  if (!Array.isArray(row[property]) || row[property].length === 0) {
    return false; // 如果 dp 不存在或为空数组，则直接返回 false
  }

  // 使用 some 而不是 map，因为 some 会在找到第一个匹配项时停止，并返回 true
  return row[property].some(i => {
    // 检查 i.childen 是否存在且不是空数组
    if (Array.isArray(i.childen) && i.childen.length > 0) {
      // 使用 some 来检查是否有任何 child 的 team 属性等于 value
      return i.childen.some(item => item.team === value);
    }
    // 如果 i.childen 不存在或为空，则继续检查下一个元素
    return false;
  });
},
