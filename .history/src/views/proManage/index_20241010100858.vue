 ...
 * @LastEditTime: 2024-10-10 10:04:54
      ...
      opsValue() {
        const ops = this.opsValues.map(item => ({
          ...
          childen: item.childen.map(child => ({
            ...child,
            // 确保每个 child 都有 ops 和 brands 属性，即使它们是空的数组
            ops: child.ops || [],
            brands: Array.isArray(child.brands) ? child.brands : [],
          })),
          // 合并 ops 和 brands（如果需要的话，这里可以自定义合并逻辑）
          // 注意：通常我们不会在这里合并它们，因为它们在级联选择器中应该是分开的层级或选项
          // 但如果您确实需要合并，请在下面的 children 处理逻辑中进行调整
        }))
        ...
        return ops.map(item => ({
          ...
          children: item.childen
            .flatMap(child => [
              ...child.ops.map(op => ({ value: op, label: op })),
              ...child.brands.map(brand => ({ value: brand, label: brand }))
            ])
          // 如果需要按某种顺序排序，可以在这里添加排序逻辑
          // .sort((a, b) => /* 排序逻辑 */)
        }))
      }
