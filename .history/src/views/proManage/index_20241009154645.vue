 ...
 * @LastEditTime: 2024-10-09 15:46:45
  ...
  export default {
    ...
    methods: {
      ...
      validateName(rule, value, callback) {
        let selectValue = []
        if (value.length < 2) {
          callback(new Error('必须选择一个店铺和一个运营'))
        } else {
          value.forEach(item => {
            selectValue.push(item)
            if (selectValue.includes(item)) {
              console.log(selectValue, item,'ae')
              callback(new Error('必须选择选择同一个商号下运营与店铺'))
            }
          })
        }
        ...
        //   console.log(storeValue, categoryValue, itemValue, 'path')
        //   if (categoryValue === '运营') {
        //     opSet.add(itemValue)
