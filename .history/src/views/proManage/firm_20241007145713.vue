 ...
 * @LastEditTime: 2024-10-07 14:49:40
...
<template>
  <div class="app-container haplotype-detail default-scrollbar">
    ...
    <el-table :data="tableData" ref="table" border style="width: 100%" @selection-change="handleSelectionChange"
      ...
      <el-table-column v-if="tableData.length!=0" prop="site1" label="站点" :filters='siteFilterValue'
        :filter-method="siteFilter" width="100">
      </el-table-column>
      <el-table-column v-if="tableData.length!=0" prop="name" label="商号">
      </el-table-column>
      <el-table-column v-if="tableData.length!=0" prop="brands" label="品牌" :filters='brandFilterValue' :filter-method="brandFilter"  v-if="this.tableData.length!=0">
        <template slot-scope="scope">
          <el-tag type="info" style="margin-right: 5px;margin-top: 5px;" v-for="op in scope.row.brands"
            :key="op">{{op}}</el-tag>
        </template>
      </el-table-column>
      <el-table-column v-if="tableData.length!=0" prop="ops" label="运营" width="300">
        <template slot-scope="scope">
