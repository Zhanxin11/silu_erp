 ...
 * @LastEditTime: 2024-10-09 15:50:56
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      validateName(rule, value, callback) {
        ...
        const selectedItems = this.convertToSelectedItems(value);

  // 检查是否选择了至少一个运营和一个店铺，并且它们来自同一商号
  ...
  selectedItems.forEach(item => {
    if (item.role === '运营') {
      operatorIds.add(item.storeId);
    } else if (item.role === '店铺') {
      storeIds.add(item.storeId);
    }

    // 如果已经找到了一个运营和一个店铺，并且它们来自同一商号
    if (operatorIds.size === 1 && storeIds.size === 1 && operatorIds.has(storeIds.values().next().value)) {
      hasValidSelection = true;
    }
  });

  // 准备验证结果
  const isValid = hasValidSelection && operatorIds.size > 0 && storeIds.size > 0;
  const errorMessage = isValid ? '' : '必须选择相同商号下的一个运营和一个店铺';
      }
  // 调用回调函数
