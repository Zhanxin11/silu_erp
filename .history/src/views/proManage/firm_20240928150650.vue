 ...
 * @LastEditTime: 2024-09-28 15:05:02
...
<script>
  import { getFirmList, getFirmLine, getFirmSelectList, delFrimListApi } from '@/api/manage'
  export default {
    components: {},
    props: {},
    data() {
          ...
          children: dpItem.childen.map(teamValue => (
            {
              ...
              children: teamValue.c?.map(unitValue => ({
                value: unitValue,
                label: unitValue
              }))
            }
    ...
    methods: {
      ...
      teamFilter(value, row, column) {
        ...
        return row.dp.some(i => {
          console.log(i.dp, i.childen, 'i.dp')
          if (i.dp) {
            console.log(i, i)
            // i.childen.some(item => item.team === value)
          }
