 ...
 * @LastEditTime: 2024-10-24 15:29:04
...
<template>
  <div class="app-container haplotype-detail default-scrollbar">
    ...
    <el-dialog class="dialog-box" :title="spuFromValues.id?'修改' :'新增'" width="36%" :visible.sync="spuDialog">
      <el-form :inline="true" label-width="80px" :model="spuFromValues" ref="spuFromValues" :rules="spuFromRules">
        <el-form-item prop="category" label="大类">
          <el-select v-model="spuFromValues.category" filterable allow-create clearable>
        ...
        <el-form-item prop="category1" label="产品线">
          <el-select v-model="spuFromValues.category1" placeholder="请输入" allow-create filterable collapse-tags
            clearable>
        ...
        <el-form-item prop="spu" label="spu">
          <el-select v-model="spuFromValues.spu" placeholder="请输入" allow-create filterable collapse-tags clearable>
  ...
  export default {
    ...
    methods: {
      ...
      validateCategory2(rule, value, callback) {
        console.log(this.spuFromValues.category1, value)
      ...
      delSpuList() {
        if (this.delId.length < 1) {
          ...
          delProListApi(this.delId).then(response => {
