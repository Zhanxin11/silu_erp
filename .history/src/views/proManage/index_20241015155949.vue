 ...
 * @LastEditTime: 2024-10-15 15:52:21
      ...
      submitUpload() {
        ...
        if (!!this.file.file) {
          ...
          postAddProApi(formData).then(response => {
            ...
            //   window.open(url)
            // }
            console.log('http://192.168.5.174:8000/' + response.file_url)
            downloadExcelEG() {         const fileUrl = 'http://192.168.5.174:8000/statics/data/sku上传模板.xlsx' // 替换为你的文件路径         window.location.href = fileUrl       },点击显示不安全路径、
            this.uploadProDialog = false
            this.downloadLoading.close()
            this.initProData()

          }).catch(error => {
            console.error('请求失败:', response, '1111')
            if (error.response) {
              console.error('状态码:', error.response.status)
            }
            this.uploadProDialog = false
