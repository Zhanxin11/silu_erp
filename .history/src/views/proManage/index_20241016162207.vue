 ...
 * @LastEditTime: 2024-10-16 16:21:43
...
<template>
  <div class="app-container haplotype-detail default-scrollbar">
    ...
    <el-table :data=" tableData" id="table_out" border style="width: 100%" :header-cell-style="tableHeaderColor"
      @select="selectPro" v-loading="loadingPro" show-summary :summary-method="proSum">
      ...
      <el-table-column prop='status' label="产品状态" :filters='statusFilterValue' :filter-method="filterStore">
        <template slot-scope="scope">
          <el-tag :type="getTagType(scope.row.status)">{{scope.row.status}}</el-tag>
          </span>
        </template>
      </el-table-column>
      <el-table-column prop='eidt' class="edFixde" fixed="right" label="操作" width="150" align="center">
        <template slot-scope="scope">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      resetChange(data) { console.log(data, 'd') },
      proSum({ columns, data }) {
        const proSum = []
        let isShow = 0
...
原文链接：https://blog.csdn.net/weixin_46971190/article/details/109337805
  .el-table {
