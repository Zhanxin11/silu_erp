 ...
 * @LastEditTime: 2024-10-08 15:05:32
...
<template>
  <div class="app-container haplotype-detail default-scrollbar">
    ...
    <el-table :data="tableData" border style="width: 100%" :header-cell-style="tableHeaderColor">
      ...
      <!-- <el-table-column label="图片" prop='pic_field' align="center" width="100">
        <template slot-scope="scope">
          <div style="height: 100px; display: flex;justify-content: center; ">
            <el-image :src="'http://192.168.5.174:8000' + scope.pic_field" fit="fill"
              :preview-src-list="'http://192.168.5.174:8000' + scope.row.pics"></el-image>
          </div>
        </template>
      </el-table-column> -->
      ...
      <el-table-column prop='status_name' label="产品状态">
        <template slot-scope="scope">
          <el-tag :type="getTagType(scope.row.status_name)">{{scope.row.statu}}</el-tag>
          </span>
        </template>
      </el-table-column>
    ...
    methods: {
      ...
      async initProData() {
        ...
        this.tableData.map(item => {
          if (item.pic_pre) {
            item.pic_pre = ['http://121.41.171.227:8000/' + item.pic_pre]
          } else {
            item.pic_pre = []
          }
