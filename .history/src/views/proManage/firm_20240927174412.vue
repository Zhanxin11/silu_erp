 ...
 * @LastEditTime: 2024-09-27 17:43:52
  ...
  export default {
    ...
    methods: {
      ...
      unitFilter(value, row, column) {
        const property = column['property']
        // 遍历 row[property]（即 dp 数组）
        for (const item of row[property]) {
          // 遍历每个 item 的 childen 数组
          for (const child of item.childen) { // 注意：这里可能是 'children' 的拼写错误
            // 检查 child 的 team 属性是否等于给定的 value
            if (child.team === value) {
              // 如果找到匹配的 team，则返回 true
              return true
            }
          }
        }
        // 如果没有找到匹配的 team，则返回 false
        return false
      },
      delFirmList() { },
      // 导入excel
