 ...
 * @LastEditTime: 2024-10-24 11:30:32
...
<template>
  <div class="app-container haplotype-detail default-scrollbar">
    ...
    <el-dialog class="dialog-box" title="新增品类" width="38%" :visible.sync="spuDialog">
      <el-form :inline="true" label-width="80px" :model="spuFromValues" ref="spuFromValues" :rules="spuFromRules">
        <el-form-item prop="category" label="大类">
          <el-select v-model="spuFromValues.category">
            <el-option v-for="(i,index) in catSelectForm" :key="index" :label="i.category" :value="i.c">
            </el-option>
          </el-select>
        </el-form-item>
        <el-form-item prop="category1" label="pingxian">
          <el-select v-model="spuFromValues.category" placeholder="产品状态" filterable collapse-tags>
            <el-option v-for="(item,index) in categoryValue" :key="index" :label="item" :value="item">
            </el-option>
          </el-select>
        </el-form-item>
      </el-form>
