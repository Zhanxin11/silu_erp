 ...
 * @LastEditTime: 2024-10-24 15:18:28
...
<template>
  <div class="app-container haplotype-detail default-scrollbar">
    <el-row style="padding-bottom: 20px">
      ...
      <el-button type="primary" @click="downloadExcelEG">上传模板示例</el-button>
      <el-button type="primary" @click="downloadExcel">下载</el-button>
      <el-button type="info" @click="addpro">新增</el-button>
      <el-col :span="3" style="float: right;">
        <el-tag>筛选条数：{{tableSum}}</el-tag>
        <span style="padding: 5px;"> <el-tag>总条数：{{tableData.length}}</el-tag></span>
      </el-col>
    </el-row>
    <el-table :data=" tableData" id="table_out" ref="table" border style="width: 100%"
      :header-cell-style="tableHeaderColor" @select="selectPro" v-loading="loadingPro"
      @filter-change="proFilterChange()">
      <!-- <el-table :data=" tableData.slice((currentPage - 1) * pageSize, currentPage * pageSize)" id="table_out" border
      style="width: 100%" :header-cell-style="tableHeaderColor" @select="selectPro" v-loading="loadingPro" show-summary
      :summary-method="proSum" @filter-change="handleFilterChange" :height="height"> -->
      ...
      <el-table-column label="图片" prop='pic_pre' align="center" width="120">
        <template slot-scope="scope">
          <div style="height: 100px; display: flex;justify-content: center; ">
            <el-image :src="'http://121.41.171.227:8000' +scope.row.pic_pre" fit="fill"
    ...
    methods: {
      ...
      async initProData() {
        ...
        this.tableData.map(item => {
          if (item.pic_field) {
            item.pic_field = ['http://121.41.171.227:8000' + item.pic_field]
          } else {
            item.pic_field = []
          }
        })
      ...
      submitUpload() {
        ...
        if (!!this.file.file) {
          ...
          postAddProApi(formData).then(response => {
            ...
            //   window.open(url)
            // }
            let fileUrl = 'http://121.41.171.227:8000/' + response.file_url
      ...
      // 下载上传模板示例
      downloadExcelEG() {
        const fileUrl = 'http://121.41.171.227:8000/statics/data/sku上传模板.xlsx' // 替换为你的文件路径
        window.location.href = fileUrl
      },
      // 表头筛选
      ...
      proFilterChange() {
        this.tableSum = this.$refs.table.tableData.length
      ...
      setHeight() {
        this.$nextTick(() => {
          this.height = innerHeight - 180 // 这里的X就是顶部栏的高度
        })
      }
    },
    created() {
