 ...
 * @LastEditTime: 2024-09-11 13:19:20
...
<template>
  <div class="app-container haplotype-detail default-scrollbar">
    ...
    <el-dialog title="新增商号" :visible.sync="dialog" width="50%" :inline="true">
      <el-form :inline="true" label-width="80px" :model="firmForm">
        <el-form-item prop="site1" label="站点">
          <el-select v-model="firmForm.site1" placeholder="请选择站点" @change="handleChange">
            <el-option v-for="(item,index) in site1" :key="index" :label="item.name" :value="item.value">
              {{item.name}}
            </el-option>
          </el-select>
        </el-form-item>
