 ...
 * @LastEditTime: 2024-10-16 11:04:17
...
<template>
  <div class="app-container haplotype-detail default-scrollbar">
    ...
    <el-table :data="tableData" id="table_out" border style="width: 100%" :header-cell-style="tableHeaderColor"
      ...
      <el-table-column prop='sell_date' label="开售日期" width="100" :filters='dateFilterValue'
        :filter-method="filterStore">
      </el-table-column>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      dateFilterValue() {
        const uniqueCategories = this.copyTableData.reduce((acc, i) => {
          const exists = acc.some(item => item.text === i.sell_date)
          if (!exists) {
            acc.push({ text: i.sell_date.substring(0, 4), value: i.sell_date.substring(0, 4) })
          }
          return acc
        }, [])
        uniqueCategories.fileSize
        return uniqueCategories
      }
    },
    methods: {
