 ...
 * @LastEditTime: 2024-09-24 16:31:47
    ...
    <el-dialog title="新增商号" :visible.sync="dialog" width="35%" :inline="true">
      <el-form :inline="true" label-width="80px" :model="firmForm">
        ...
        <el-form-item prop="status" label="团队状态">
          ...
          <el-cascader :options="dpValue"></el-cascader>
          <div>{{teamValue}}</div>
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
        <el-button @click="dialog = false">取 消</el-button>
        <el-button type="primary" @click="saveAddFirm">确 定</el-button>
      </span>
    </el-dialog>
  ...
  export default {
    ...
    data() {
      return {
        ...
        [ { "value": "BG2", "label": "BG2", "children": [ { "value": [ "A" ], "label": [ "A" ], "children": [ { "value": "BU1", "label": "BU1" } ] } ] } ]
      }
    },
    watch: {},
    computed: {
      ...
      teamValue() {
        const dp = this.firmForm.dp.filter(Boolean) // 移除空字符串或 falsy 值
        const team = this.firmForm.team || 'NoTeam' // 假设只有一个团队，或默认为 'NoTeam'
        const units = this.firmForm.unit // 假设这是对应团队的单位列表

        // 构建数据结构
        return dp.map(dpValue => ({
          value: dpValue,
          label: dpValue,
    ...
    methods: {
      ...
      // 保存新增
      saveAddFirm() {
        console.log(this.firmFormValues)
        this.dialog = false
      },
      // 多选
