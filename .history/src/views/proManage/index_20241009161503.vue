 ...
 * @LastEditTime: 2024-10-09 16:00:50
  ...
  export default {
    ...
    methods: {
      ...
      validateName(rule, value, callback) {
        ...
        // }
        const selected = value || []
        const storeOpPairs = [] // 用于存储匹配的运营和店铺对
        const storeSet = new Set() // 用于存储所有选中的店铺，以检查重复
        const opSet = new Map() // 使用Map来存储运营，其中键是商号，值是一个Set，包含该商号下的所有运营

        selected.forEach(path => {
          const [storeId, category, itemValue] = path // 假设storeId是商号
          if (category === '运营') {
        ...
        // 验证规则
        if
        else(storeOpPairs.length < 1) {
          callback(new Error('必须选择一个店铺和一个同商号下的运营人员'))
