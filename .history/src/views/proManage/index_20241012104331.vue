 ...
 * @LastEditTime: 2024-10-12 10:43:00
  ...
  <div class="app-container haplotype-detail default-scrollbar">
    ...
    <el-dialog title="新增" :visible.sync="dialogVisible" width="50%" :inline="true">
      <el-form :inline="true" label-width="80px" :model="proFormValues" ref="proFormValues" :rules="proRules">
        ...
        <el-form-item prop="brand_name" label="品牌" width="350px">
          <el-select v-model="proFormValues.brand_name" placeholder="请先选择商号" filterable collapse-tags style="width:92%">
            <el-option v-for="(item,index) in brand" :key="index" :label="item" :value="item">
            </el-option>
          </el-select>
        </el-form-item>
        <el-form-item prop="op_name" label="运营" width="350px">
          <el-select v-model="proFormValues.op_name" placeholder="请先选择商号" filterable collapse-tags>
        ...
        <el-form-item prop="status_name" label="产品状态" width="350px">
          <el-select v-model="proFormValues.status_name" placeholder="产品状态" filterable collapse-tags>
            <el-option v-for="(item,index) in proForm.status" :key="index" :label="item" :value="item">
            </el-option>
          </el-select>
        </el-form-item>
        <el-form-item prop="pic_field" label="图片" width="50%">
          <el-upload class="upload-demo" action="#" :on-preview="handlePreview" :on-remove="handleRemove"
            ...
            :before-upload="beforeUpload" on-change="changePic" accept=".jpg,.jpeg,.png" :auto-upload="false" show-file-list>
...
<script>
  import { getProductList, getProSelectList, postAddProApi } from '@/api/manage'
  export default {
    ...
    data() {
      return {
        ...
        proFormValues: {
          ...
          fasin: 'B0C7W233FS',
          asin: 'B08DXG7V1S',
          sASInL: 'C003-9-Clear-100-3',
          sku: '塑料杯-9 oz-100pack-透明杯',
        ...
        proRules: {
          ...
          brand_name: [{ required: true, msg: '请选择店铺', trigger: 'change' }],
          op_name: [{ required: true, msg: '请选择运营', trigger: 'change' }],
          category: [{ required: true, msg: '请输入大类', trigger: 'change' }],
          category1: [{
            required: true, validator: (rule, value, callback) => {
              // const regex = /^大类-\w+$/ // 简单的正则表达式示例，您可能需要根据实际格式调整
              // if (!value) {
              //   callback(new Error(rule.message))
              // } else if (!regex.test(value)) {
              //   callback(new Error('格式不正确，应为:大类-XX'))
              // } else {
              callback()
              // }
            }, trigger: 'blur'
          ...
          spu_name: [{ required: true, validator: this.validateSpu, trigger: 'blur' }],
          ...
          status_name: [{ required: true, msg: '请输细分线', trigger: 'change' }],
        }
      }
    },
    watch: {
    ...
    methods: {
      ...
      // 保存新增
      saveAddPro(formName) {
        console.log(formName, this.proFormValues, 111)
      ...
      handleRemove(file, fileList) {
        this.proFormValues.pic_field = file
