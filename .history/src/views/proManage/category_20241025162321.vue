 ...
 * @LastEditTime: 2024-10-25 16:20:11
...
<template>
  <div class="app-container haplotype-detail default-scrollbar">
    ...
    <el-dialog class="dialog-box" :title="level==1?'修改大类':'修改产品线'" width="34%" :visible.sync="changeDialog">
      ...
      <span slot="footer">
        <el-button @click="save('spuFromValues')" type="primary">确认修改</el-button>
    ...
    methods: {
      ...
      saveSpu(formName) {
        for (const key in this.spuFromValues) {
          ...
          this.spuFromValues[key].replace(/^\s*|\s*$/g, '')

        }
        // this.spuFromValues.map(i => {
