 ...
 * @LastEditTime: 2024-09-11 13:43:25
    ...
    <el-dialog title="新增商号" :visible.sync="dialog" width="35%" :inline="true">
      <el-form :inline="true" label-width="80px" :model="firmForm">
        <el-form-item prop="site1" label="站点">
          <el-select v-model="firmForm.site1" placeholder="请选择站点" filterable>
            <el-option v-for="(item,index) in firmForm.site1" :key="index" :label="item" :value="item">
              {{firmForm.site1}}
            </el-option>
          </el-select>
          {{firmForm.site1}}
