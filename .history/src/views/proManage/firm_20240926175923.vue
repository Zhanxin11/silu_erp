 ...
 * @LastEditTime: 2024-09-26 17:58:52
  ...
  <div class="app-container haplotype-detail default-scrollbar">
    ...
    <el-table :data="tableData" border style="width: 100%" @selection-change="handleSelectionChange"
      ...
      <el-table-column prop="dp" label="部门" :filters='dpFilterValue' :filter-method="opFilter">
        <template slot-scope="scope">
          <el-tag style="margin-right: 5px;margin-top: 5px;" v-for="k in scope.row.dp" key="k">{{ k.dp
            }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column prop="dp" label="团队" :filters='teamFilterValue' :filter-method="teamFilter">
        <template slot-scope="scope">
          <span style="margin-right: 5px;margin-top: 5px;" v-for="op in scope.row.dp" :key="op">
            <span v-for="i in op.children" :key="i">{{i.team}}</span>
          </span>
        </template>
      </el-table-column>
      <!-- <el-table-column prop="unit1" label="小组" :filters='unitFilterValue' :filter-method="unitFilter">
        <template slot-scope="scope">
          <span style="margin-right: 5px;margin-top: 5px;" v-for="item in scope.row.dp" :key="item">
            <span v-for="i in item.children" :key="i">{{i.children}}</span>
          </span>
        </template>
      </el-table-column> -->
    ...
    <el-dialog title="新增商号" :visible.sync="dialog" width="35%" :inline="true">
      <el-form :inline="true" label-width="80px" :model="firmFormValues" ref="firmFormValues" :rules="frimRules">
        ...
        <el-form-item prop="statusValue" label="团队状态">
  ...
  export default {
    ...
    data() {
      return {
        ...
        frimRules: {
          ...
          nameValue: [{ required: true, message: '请选择商号', trigger: 'blur' },
          ],
          brandValue: [{ required: true, message: '请选择品牌', trigger: 'blur' },
          ],
          opValue: [{ required: true, message: '请选择部门', trigger: 'blur' },
          ],
          dpValue: [{ required: true, message: '请选择部门', trigger: 'blur' },
          ],
          statusValue: [{ required: true, message: '请选择团队状态', trigger: 'blur' },
          ],
        },
      }
      // unit2: [{
    ...
    computed: {
      ...
      teamValue() {
        ...
        const team = this.firmForm.team
        const units = this.firmForm.unit
    ...
    methods: {
      ...
      saveAddFirm(formName) {
        console.log(this.firmFormValues, this.$refs[formName])
