 ...
 * @LastEditTime: 2024-10-26 09:29:09
...
export function getCatSelectApi() {
  return request({
    url: '/silu/spuview/get_dropdown/',
    method: 'get'
  })
}
// 添加品类
export function postAddCatApi(data) {
  const cat = new FormData()
  for (let key in data) {
    if (data.hasOwnProperty(key)) {
      cat.append(key, data[key])
    }
  }
  return request({
    url: '/silu/spuview/',
...
// 修改品类
export function putChangeCatApi(data) {
  const cat = new FormData()
  llo
  for (let key in data) {
    if (data.hasOwnProperty(key) && key != 'id') {
  ...
  return request({
    url: `/silu/spuview/${data.id}/`,
    method: 'put',
    data: cat
  })
}
export function delCatListApi(data) {
  const delCat = new FormData()
  for (let i = 0; i < data.length; i++) {
    delCat.append(i, data[i])
  }
  return request({
    url: '/silu/spuview/0/',
    method: 'delete',
...
export function putChangeCatFatherApi(data) {
  const cat = new FormData()
  for (let key in data) {
    cat.append(key, data[key])
